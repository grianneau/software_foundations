...
(** Here, we could finish with "[rewrite -> eq.  reflexivity.]" as we
  ...
  exact: eq. (* or alternatively 'by apply: eq.' *)
Qed.
...
(** If we simply tell Coq [apply trans_eq] at this point, it can
  ...
  apply: (trans_eq _ _ _ _ eq1).
  exact: eq2.
Qed.

(** Coq is often smart enough to figure out which variable we
    are instantiating. The term [trans_eq] has type 
    [forall (X : Type) (n m o : X), n = m -> m = o -> n = o].
    This means that [trans_eq] can be applied to a first argument
    which has type [Type]. Let's call this first argument [X] 
    as in the displayed type of [trans_eq]. Then, the term [trans_eq X]
    can be applied to a second argument which has type [X].
    And so on, [trans_eq] can be applied to up to 6 arguments.
    When [trans_eq] is applied to 6 arguments (with the right types)
    the resulting term has type [n = o] where [n] is the second argument
    and [o] is fourth argument.
    The first four [_] are telling to the Coq system that it should try 
    to find the values for [X], [n], [m] and [o]. The fifth argument
    is [eq1] and from this Coq can infer that [n] is [[a; b]] and
    [m] is [[c; d]]. Thus Coq can infer that [X] is [list nat].
    The term [trans_eq _ _ _ _ eq1] has the type [n = o] and Coq
    tries to apply it to the goal. It infers that [o] is [[e; f]].
    The only one thing missing that Coq cannot infer is a term
    with type [[c; d] = [e; f]]. The term [eq2] is exactly such a term.
 *)
...
Proof.
  move=> a b c d e f eq1 eq2.
  rewrite eq1.
  rewrite eq2. (* alternatively: by rewrite eq2. *)
  by [].
Qed.
...
Proof.
  ...
  injection H.
  move=> H1 H2. 
  (* Both above lines can be combined into: injection H=> H1 H2.*)
  rewrite H1. 
  rewrite H2. 
  by [].
Qed.
...
Proof.
  move=> n m contra.
  (* There is a variable of type 'false = true' in the context. *)
  (* In this case or if there is a variable of type False, *)
  (* then we can ask the system to kill the goal. *)  
  by [].
Qed.
...
Proof.
  move=> n contra.
  (* There is a variable of type 'S _ = 0' in the context. *)
  (* This is hopefully impossible. *)
  (* We can ask the system to kill the goal: *)  
  by [].
Qed.
...
(** Coq can see that the subgoal we are working on is impossible: *)
...
(** The SSReflect tactic [apply:] rewrites a goal
    matching [Y] into a subgoal [X]). The colon differentiates
    the SSReflect tactic from the original [apply] tactic.
    This way, both tactics are available (un like the [rewrite] tactic).

    By contrast, [apply: L] is "backward reasoning": it says that if we
    know [X -> Y] and we are trying to prove [Y], it suffices to prove
    [X]. When developing proof scripts in Coq, "backward reasoning"
    is prioritised over "forward reasoning" because working on the goal
    makes proof scripts more robust.
...
Proof.
  ...
  - (* n = O *) 
    rewrite /=.
    case: m => [eq| m' eq]. (* alternatively: case: m => [| m'] eq. *)
    + (* m = O *) by [].
    + (* m = S m' *) by [].
  - (* n = S n' *)
    move: IHn'. 
...
Proof.
  ...
  - (* n = O *) move=> m.
                rewrite /=.
                (* Both above lines can be replaced with: move=> m /=. *) 
                case: m => [| m'] /= eq.
    + (* m = O *) by [].
    + (* m = S m' *) by [].
...
(** **** Exercise: 3 stars, standard, especially useful (plus_n_n_injective)
    ...
    In addition to being careful about how you use introductions
    with [move=>], practice using "in" variants in this proof.  
    (Hint: use [plus_n_Sm].) *)
...
(** The problem is that, to do induction on [m], we must first
    introduce [n].  (And if we simply say [elim: m] without
...
(** What we can do instead is to first introduce all the quantified
    ...
    back at the beginning of the goal.  The [move:]
    tactic does this. *)
...
(** ... we appear to be stuck: [rewrite /=] doesn't simplify anything, and
...
Proof.
  move=> m.
  move=> /=.
  (* Both above lines can be combined into: move=> m /=. *)
  by [].
Qed.
...
(** The reason that [rewrite /=] doesn't make progress here is that it
...
(** At this point, there are two ways to make progress.  One is to use
    [case: m] to break the proof into two cases, each focusing on a
...
Proof.
  move=> m.
  case: m=> [|m'] /=.
  (* Both above lines can be combined into: move=> m; case: m=> [|m'] /=. *)
  - by [].
  - by [].
Qed.
...
(** After unfolding [sillyfun] in the above proof, we find that
    ...
    [n] is equal to [3] or it isn't, so we can use [case: (eqb
...
(** The [H] part of the [case H:] tactic is optional; 
    we've chosen not to include it when it is not necessary.

    However, when destructing compound expressions, the information
    recorded in [H] can actually be critical: if we leave it
    out, then [case:] can erase information we need to complete a
    proof.
...
(** ... then we are stuck at this point because the context does
    ...
    that the substitution performed by [case:] is quite brutal --
    ...
    records which case we are in.  This equation is precisely [H] in [case H:]. *)
...
Proof.
  ...
  (** Now we have the same state as at the point where we got
    ...
    - (* e3 = true *) 
      move=> _.
      move/eqb_true: Heqe3 => Heqe3.
      rewrite Heqe3. 
      (* Both above lines can be combined into:
      move/eqb_true: Heqe3 => ->. *)
      by [].
    - (* e3 = false *)
...
(** We've now seen many of Coq's most fundamental tactics.  We'll
    ...
    Here are the ones we've seen:
      ...
      - [by []]: finish the proof (when the goal looks like [e =
        e])

      - [apply:]: prove goal using a hypothesis, lemma, or constructor

      - [rewrite /=]: simplify computations in the goal

      - [rewrite /= in H]: ... or a hypothesis (to be avoided if possible, 
        because we don't like the script to rely on an introduced name; 
        we prefer to work on the goal instead)

      - [rewrite]: use an equality hypothesis (or lemma) to rewrite
        the goal

      - [rewrite ... in H]: ... or a hypothesis (same remark as for 
        [rewrite /= in H])
      ...
      - [rewrite /name]: replace a defined constant by its right-hand side in
        the goal

      - [rewrite /name... in H]: ... or a hypothesis

      - [case: ... => [...]]: case analysis on values of inductively
        defined types

      - [case H : ... => [...]]: specify the name of an equation to be
        added to the context, recording the result of the case
        analysis

      - [elim:]: induction on values of inductively
        defined types

      - [injection...]: reason by injectivity on equalities
        between values of inductively defined types

      - [case:]: reason by disjointness of constructors on
        equalities between values of inductively defined types

      - [have H: e]: introduce a "local lemma" [e] and call it [H]

      - [move: x]: move the variable [x] to an explicit hypothesis 
        in the goal formula. This cannot be performed if something
        from the context depends on x. In this case one can first 
        move other parts of the context in the goal.
