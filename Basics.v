...
(** * Tactic language
    ...
    In addition to the _Gallina_ language mentioned above, we will use a language 
    to develop proofs - more precisely, to write proof scripts.
    We use the proof language when we are in the proof mode. The latter starts
    after the keyword [Proof]. The default proof mode is "Classic" and comes with
    the [Ltac] tactic language. In this mode, additional tactics are available 
    after loading plugins such as SSR or micromega. When the module Prelude -
    corresponding to the file Init/Prelude.v - is loaded, the proof mode 
    is set to “Classic”. (In the ”Noedit” mode, no tactic language is activated.)
    We work in the default mode and loads the SSReflect plugin
    with the following command. *)

From Coq Require Import ssreflect.   

(** 
    In all subsequent files, we will start with this command to load the SSReflect plugin
    and activate the SSReflect set of tactics.
    One can still use some of the tactics that were available before this command is executed,
    but some are overwritten. For instance, the behaviour of the [rewrite] tactic 
    is modified.
    We will try to use the set of tactics that comes with loading the SSReflect plugin,
    whenever possible.
    This set of tactics has the advantage that it is smaller and one can do 
    most interactive reasoning steps with it. This way, there are fewer tactics 
    to remember to develop proofs with Coq. *)
...
(** (We show Coq's responses in comments, but, if you have a
    ...
    Coq interpreter under your favorite IDE -- jsCoq, CoqIde, 
    Visual Studio Code or Proof General -- and try it for yourself.  
    Load this file, [Basics.v], from the book's Coq sources, 
    find the above example, submit it to Coq, and observe the result.) *)
...
(** The [Locate] command displays the full name of the object 
    from Coq's various qualified namespaces. *)

Locate bool.

(** We see that the booleans [bool] are defined both in [LF.Basics]
    (our current file) and in [Coq.Init.Datatypes]. 
    The [Locate] command also displays notation definitions. *)

Locate "if".

(** We see that the notation [if] [then] [else] 
    is already defined, but for the type [Coq.Init.Datatypes.bool]. 
    We can define the same notation for our boolean type [bool]. *)
...
(** A better solution is to use the notation [if] [then] [else]
    in the scope general_if_scope. There are two ways to use it. We can
    open it locally by writing the code within [( ... )%GEN_IF].
    An other solution is to open thi scope with the command
...
(** (You may notice if you step through these proofs that
    ...
    done by [by []].  We'll discuss why that is shortly.)
...
(** Now that we've defined a few datatypes and functions, let's
    ...
    were always the same: use [rewrite /=] to simplify both sides of the
    equation, then use [by []] to check that both sides contain
    identical values. More generally, the [by []] tactic (equivalent
    to the tactic [done]) tries to solve the current goal by some trivial means
    (and fails if it doesn't succeed).
...
(** (You may notice that the above statement looks different in
    ...
    This is a good place to mention that [by []] is a bit more
    powerful than we have acknowledged. In the examples we have seen,
    the calls to [rewrite /=] were actually not needed, because
    [by []] can perform some simplification automatically when
...
(** Moreover, it will be useful to know that [by []] does
    somewhat _more_ simplification than [rewrite /=] does -- for example,
    ...
    to look at whatever expanded expressions [by []] has created
    by all this simplification and unfolding; by contrast, [rewrite /=] is
...
(** The keywords [intros], [rewrite /=], and [by []] are examples of
...
(** It is worth stepping through these proofs to observe how the
    context and the goal change.  You may want to add calls to [rewrite /=]
    before [by []] to see the simplifications that Coq performs
...
(** Of course, not everything can be proved by simple
    ...
    fact using the [rewrite /=] tactic as above, we get stuck.  (We then
...
(** The [destruct] generates _two_ subgoals, which we must then
    ...
    use of [by []], which itself performs some simplification --
...
(** Each pair of calls to [by []] corresponds to the
